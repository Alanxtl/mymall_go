...
func AddItem(ctx context.Context, db *gorm.DB, cartItem *Cart) error {
	var row Cart
	err := db.WithContext(ctx).
		Model(&Cart{}).
		Where(&Cart{UserId: cartItem.UserId, ProductId: cartItem.ProductId}).
		First(&row).
		Error
	...
	if row.ID > 0 {
		return db.WithContext(ctx).
			Model(&Cart{}).
			Where(&Cart{UserId: cartItem.UserId, ProductId: cartItem.ProductId}).
			UpdateColumn("Quantity", gorm.Expr("Quantity+?", cartItem.Quantity)).
			Error
	}

	return db.WithContext(ctx).Create(cartItem).Error
}
